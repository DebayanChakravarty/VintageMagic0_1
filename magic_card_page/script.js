document.addEventListener('DOMContentLoaded', () => {
    ...
    dynamicCards.style.backgroundImage = `url("../assets/${images[greetingCardData.backgroundImageId]}.webp")`;
    ...
    const sendIt = document.getElementById('sendButton')
    sendIt.addEventListener('click',function(){
        const message = {
            from: greetingCardData.from,
            to: greetingCardData.to,
            subject: greetingCardData.subject,
            message: greetingCardData.message,
            timestamp: greetingCardData.timestamp,
            greetingType: greetingCardData.greetingType,
            backgroundImageId:greetingCardData.backgroundImageId,
            token:1234,
        };


        fetch('https://vinmagic.bydeb.in/V1/creategreetingsInfo', {
            method: 'POST',
            headers: {
